...
class Com:
    ...
    def ask_station_info(self, station_number):
        ...
        answer = self.ask(3, qid=station_number)
        answer = answer.split(':')
        if int(answer[0]) == -1:
    ...
    def is_outdoor(self, position):
        if self.ask(6, input1=position)[0] == 't':
            return True
        return False

    def is_pos_direct(self, position1, position2):
        if self.ask(7, input1=position1, input2=position2) == 't':
            return True
        return False

    def ask(self, question, qid=None, input1=None, input2=None):
        # Questions:
        # Q1: How many stations are?
        # Q2: How many users are?
        # Q3: Ask information of base station #N
        # Q4: Ask information of user #N
        # Q6: Ask if the position is out door
        # q7: Ask if the position_1 is direct path position_2.
        question = int(question)
        message = 'q{0}'.format(question)
        if qid is not None:
            message += ':{1}'.format(qid)
